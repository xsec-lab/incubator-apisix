...
local function ews_check(conf, ctx)
    ...
    if conf.debug then
        core.log(string.format("username: %s, user_agent: %s, ip: %s", username, user_agent, remote_ip))
    end
    ...
    ngx.log(ngx.INFO, string.format("result: %s, ews_address_status: %s, ews_status: %s", result, ews_address_status, ews_status))
...
local function ews(conf, ctx)
    ...
    if username ~= "" then
        ...
        is_office_wlan = false

        if core.strings.startswith(ngx.var.uri, "/EWS/") then
            if is_office_vlan or is_office_wlan then
                -- 如果是内网地址或公司出口IP，直接跳过验证逻辑
            else
                core.log(string.format("username: %s, run_mode: %s, remote_ip: %s, client_type: %s", username, conf.run_mode, remote_ip, client_type))
